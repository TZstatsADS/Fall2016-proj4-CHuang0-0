...
dim(lyr.dt) # 2350 5000
lyr.dt.nrow=nrow(lyr.dt);lyr.dt.nrow #2350
vocab;length(vocab) #3914

#empty a list to store values 
list1=list();list1
for(i in 1:lyr.dt.nrow){
  ind1=which(lyr.dt[i,] != 0)  # get all the words from the lyr data that exists in the original text
  nam1=Dic[ind1]
  match_1=match(nam1,vocab)
  ind2=match_1[!is.na(match_1)]
  doc1=rbind(ind2, rep(1,length(ind2)))
  list1[[i]]=doc1
 }
list1
