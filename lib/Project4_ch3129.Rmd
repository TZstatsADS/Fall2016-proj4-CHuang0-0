...
t2 <- Sys.time()
t2 - t1  # about 4.011305 minutes on laptop
...
#store results
write.csv(theta, file = "Theta(para=0.1,15topic).csv",row.names=FALSE)
write.csv(phi, file = "Phi(para=0.1,15topic).csv",row.names=FALSE)
# also tried other tuning parameters. dont differ much.
```
...
ind.inf.feat=which(abs(mat.anal.feat.colmean) == Inf)
